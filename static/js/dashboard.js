...
// 홈 위젯은 카드 전체가 드래그 대상이므로 모바일 스크롤 제스처가 재배치로
// 오인되지 않게 기본 상태를 잠금으로 둔다. 배치 편집이 필요할 때만 사용자가
// 명시적으로 잠금을 해제하며, 이 기기 전용 UI 상태는 localStorage에 저장한다.
...
function setHomeWidgetsLocked(locked) {
  try {
    ...
    // localStorage를 사용할 수 없어도 현재 화면에는 잠금 상태를 반영한다.
  }
