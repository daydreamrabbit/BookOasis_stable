# -*- coding: utf-8 -*-
"""
video_repository.py – MariaDB 전용 영상 강좌(videos, video_episodes, video_progress, video_episode_progress) 데이터 액세스 레이어
"""
import os
import database
...
class VideoRepository:
    ...
    def update_favorite(video_id, is_favorite):
        ...
        try:
            cursor.execute(
                "UPDATE videos SET is_favorite = %s WHERE id = %s",
    ...
    def update_episode_probe_result(episode_id, video_id, duration, width, height, needs_transcode):
        """재생 시점(JIT)에 즉석으로 ffprobe한 결과를 반영하고 강좌 total_duration을 재계산한다."""
        conn = database.get_connection('video')
        cursor = conn.cursor()
        try:
            cursor.execute(
                "UPDATE video_episodes SET duration = %s, width = %s, height = %s, needs_transcode = %s WHERE id = %s",
                (duration, width, height, needs_transcode, episode_id)
            )
            cursor.execute(
                "UPDATE videos SET total_duration = (SELECT COALESCE(SUM(duration), 0) FROM video_episodes WHERE video_id = %s) WHERE id = %s",
                (video_id, video_id)
            )
            conn.commit()
        except Exception:
    ...
    def save_video_episode_progress(video_id, user_id, episode_id, current_time, progress_pct, is_completed):
        ...
        try:
            cursor.execute("""
                REPLACE INTO video_episode_progress (
        ...
        try:
            ...
            if row:
                ...
                cursor.execute(
                    ...
                    "FROM video_episodes WHERE video_id = %s",
                    (video_id,)
                )
            ...
            for p in removed_paths:
                cursor.execute("DELETE FROM video_episodes WHERE id = %s", (existing_episodes_full[p]['id'],))
                episode_deletes += 1
