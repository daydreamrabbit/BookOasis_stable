...
class UserRepository:
    ...
    def update_download_access(db_type, user_id, has_download_access):
        ...
        cursor = conn.cursor()
        try:
            cursor.execute("UPDATE users SET has_download_access = %s WHERE id = %s", (has_download_access, user_id))
            conn.commit()
            return True
        except Exception as e:
            conn.rollback()
    ...
    def update_content_rating_max(db_type, user_id, content_rating_max):
        ...
        cursor = conn.cursor()
        try:
            cursor.execute("UPDATE users SET content_rating_max = %s WHERE id = %s", (content_rating_max, user_id))
            conn.commit()
            return True
        except Exception as e:
            conn.rollback()
